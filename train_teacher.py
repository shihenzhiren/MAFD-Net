...
from torch.cuda.amp import autocot, GradScaler
...
from dataset.nsl import get_nsl_dataloaders  # 导入 nsl 数据集的加载函数
from dataset.QAX2024 import get_QAX2024_dataloaders  # 导入 QAX2024 数据集的加载函数
from dataset.unsw import get_unsw_dataloaders  # 导入 UNSW 数据集的加载函数
...
def parse_option():
    ...
    parser.add_argument('--start_epoch', type=int, default=1, help='start epoch (used with --resume)')

    # dataset
    parser.add_argument('--model', type=str, default='resnet32x4')
    parser.add_argument('--dataset', type=str, default='cifar100',
                        choices=['cifar100', 'imagenet', 'nsl', 'QAX2024', 'unsw'], help='dataset')  # 增加 unsw 数据集选项
...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    n_cls = {
        'cifar100': 100,
        'imagenet': 1000,
        'nsl': 5,  # NSL 数据集的类别数为 5
        'QAX2024': 81,  # QAX2024 数据集的类别数为 81
        'unsw': 10,     # UNSW 数据集的类别数为 10
    ...
    if opt.resume:
        if os.path.isfile(opt.resume):
            print(f"==> 加载 checkpoint: {opt.resume}")
            ...
            if isinstance(checkpoint, dict) and 'model' in checkpoint:
                ...
                if 'epoch' in checkpoint:
                    ...
                    print(f"==> 从 epoch {opt.start_epoch} 继续训练")
            else:
                # 只保存了权重，没有 epoch/optimizer 信息
                model.load_state_dict(checkpoint)
                print(f"==> 已加载权重，从 epoch {opt.start_epoch} 继续训练")
        else:
            print(f"==> 警告：找不到 checkpoint: {opt.resume}，从头开始训练")
    ...
    elif opt.dataset == 'nsl':  # 增加 nsl 数据集的处理逻辑
        train_loader, val_loader = get_nsl_dataloaders(batch_size=opt.batch_size, num_workers=opt.num_workers)
    elif opt.dataset == 'QAX2024':  # 增加 QAX2024 数据集的处理逻辑
        train_loader, val_loader = get_QAX2024_dataloaders(batch_size=opt.batch_size, num_workers=opt.num_workers)
    elif opt.dataset == 'unsw':  # 增加 UNSW 数据集的处理逻辑
        train_loader, val_loader = get_unsw_dataloaders(batch_size=opt.batch_size, num_workers=opt.num_workers)
    else:
