...
class SimKD(nn.Module):
    def __init__(self, s_n, t_n, factor=1.0, **kwargs):
        super(SimKD, self).__init__()
        self.factor = factor  # 存储 factor
        # 通道对齐模块
        self.channel_align = nn.Sequential(
            nn.Conv2d(s_n, t_n, kernel_size=1),  # 将学生模型的通道数对齐到教师模型
            nn.BatchNorm2d(t_n),
            nn.ReLU(inplace=True)
        )
        # 空间对齐模块
    ...
    def forward(self, feat_s, feat_t, cls_t):
        ...
        if feat_s.shape[1] != feat_t.shape[1]:
            feat_s = self.channel_align(feat_s)

        # 生成蒸馏特征
        trans_feat_s = feat_s  # 学生模型特征
        trans_feat_t = feat_t  # 教师模型特征

        # 预测
        temp_feat = self.avg_pool(trans_feat_s)  # [batch_size, t_n, 1, 1]

        # 动态调整 temp_feat 的通道数
        if temp_feat.shape[1] != 640:  # 假设 cls_t 需要输入通道数为 640
            temp_conv = nn.Conv2d(temp_feat.shape[1], 640, kernel_size=1).to(temp_feat.device)
            temp_feat = temp_conv(temp_feat)

        pred_feat_s = cls_t(temp_feat)  # 传递给 cls_t
        pred_feat_s = pred_feat_s.view(pred_feat_s.size(0), -1)  # 将 [64, num_classes, 1, 1] 展平为 [64, num_classes]
